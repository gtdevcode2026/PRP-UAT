...
window.Reports.s2a = async function s2a(wb) {
  ...
  // Embedded charts reproduce the original matplotlib style (white
  // background, stacked bars, white in-bar value labels, bold grand-total
  // label above each bar, bottom legend, dashed gridlines). The Open
  // Assessment chart stacks Total (blue, bottom) with Open (orange) above
  // it - Open vs Grand Total, per the user, Closed is not charted. Both
  // charts are embedded into the file even though the preview only ever
  // surfaces the first one - confirmed as the intended behavior.
  function s2aChartTraces(zoneArr) {
    var zones = zoneArr.map(function (z) { return z.zone; });
    var totalVals = zoneArr.map(function (z) { return z.grandTotal; });
    var openVals = zoneArr.map(function (z) { return z.open; });
    // The bar is Total + Open stacked; the annotation above each bar still
    // reads the Grand Total, positioned at the stacked bar's real top.
    var tops = zoneArr.map(function (z) { return z.grandTotal + z.open; });
    return {
      zones: zones, tops: tops, labels: totalVals,
      traces: [
        { x: zones, y: totalVals, type: 'bar', name: 'Total', marker: { color: '#2F75B5' },
          text: totalVals.map(function (v) { return v > 0 ? String(v) : ''; }),
  ...
  function s2aLayout(title, yTitle, zones, tops, labels) {
    var maxTop = Math.max.apply(null, tops.concat([1]));
    return {
      barmode: 'stack',
      paper_bgcolor: '#ffffff', plot_bgcolor: '#ffffff',
      title: { text: '<b>' + title + '</b>', font: { color: '#000000', size: 14 } },
      xaxis: { title: 'Zone', tickfont: { color: '#000000' } },
      yaxis: { title: yTitle, gridcolor: 'rgba(0,0,0,0.3)', griddash: 'dash', range: [0, maxTop * 1.2] },
      legend: { orientation: 'h', x: 0.5, xanchor: 'center', y: -0.25, font: { color: '#000000' } },
      annotations: zones.map(function (z, i) {
        return labels[i] > 0
          ? { x: z, y: tops[i], text: String(labels[i]), showarrow: false, yshift: 12, font: { color: '#000000', size: 9 } }
          : null;
      }).filter(Boolean),
  ...
  chartConfigs['Auto Open Closed'] = {
    traces: oc.traces,
    layout: s2aLayout(totalOpen + '/' + totalAll + ' Open Assessment', 'Assessment Count', oc.zones, oc.tops, oc.labels),
  };
  ...
  // replace the two baked PNGs. The Open Assessment chart stacks Total
  // (blue, column D) with Open (orange, column C) above it - Open vs Grand
  // Total, Closed is not charted; the Overdue chart plots OPEN overdue
  // only - an overdue assessment that is already closed is not charted.
  var placements = [];
  ...
  // Zone labels sit in column A (editable there); C=Open, D=Total.
  function s2aPlacement(sheetName, zoneArr, title, headerRow, openOnly) {
    ...
    var series = [
      { name: { lit: 'Total' },
        values: { ref: R(sheetName, 4, first, last), cache: zoneArr.map(function (z) { return z.grandTotal; }) }, color: '2F75B5' },
      { name: { lit: 'Open' },
